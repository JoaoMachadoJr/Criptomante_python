...
class ResultadosRepository(AbstractRepository):
    def cotacoes_medias_diarias(self, limite_inferior, limite_superior):
        sql = """select date_trunc('day', data) as data, (sum(valor*volume)/sum(volume)) valor, sum(transacoes) as transacoes, sum(volume) as volume
                from cotacoes c
    ...
    def gravar_previsao_numerica(self, dados, algoritmo):
        ...
        self.execute(sql)
        sql = "insert into graficos.previsao_numerica (data, valor, transacoes, volume, algoritmo) values (:data, :valor, :transacoes, :volume, :algoritmo)"
        self.executeMany(sql, [{"data": p["data"], "valor": p["valor"], "transacoes": p["transacoes"], "volume": p["volume"], "algoritmo": algoritmo} for p in dados])
        self.commit()
